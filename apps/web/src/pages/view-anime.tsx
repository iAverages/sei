...
const Login = () => {
    ...
    const animeData = createMemo(() => {
        const a = animes.data?.animes.map((anime) => ({
            ...anime,
            list_status: animes.data?.list_status.find((status) => status.anime_id == anime.id) as Status | undefined,
        }));
                    ...
                    {(anime) => (
                        <A href={`/anime/${anime.id}`}>
                            <div
                                ...
                                classList={{
                                    "scale-110 bg-blue-600": anime.id == (params.id as unknown as number),
