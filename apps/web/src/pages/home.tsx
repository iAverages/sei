...
import { AnimeCardInnerContent, DraggableAnimeCard } from "~/components/anime-card";
...
export default function Home() {
    ...
    const getEntry = (anime: Anime) => {
        return userList.data.list_entries.find(entry => entry.anime_id === anime.id)
    }
                ...
                <DragDropProvider onDragEnd={onDragEnd} collisionDetector={closestCenter}>
                    ...
                    <SortableProvider ids={ids()}>
                        <div class={"grid grid-cols-2 md:grid-cols-3 lg:grid-cols-8 gap-3"}>
                            <For each={items()}>
                                {(anime) => (<DraggableAnimeCard anime={anime} bringToFront={() => { }} disabled={userList.isLoading} />
                                )}
                            </For>
                        </div>
                    </SortableProvider>
